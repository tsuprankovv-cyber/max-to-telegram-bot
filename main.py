...
async def process_message(msg: Dict):
    ...
    # Извлечение ID (с учётом body.mid)
    msg_id = safe_str(
    ...
    # Извлечение текста (с учётом body.text)
    text = safe_str(
    ...
    if h in _processed_hashes:
        logger.debug(f"⏭ Skip duplicate: {h}")
        return
    _processed_hashes.add(h)
    if len(_processed_hashes) > 1000: _processed_hashes.clear()
    
    # Пропуск служебных сообщений
    if not msg_id:
        logger.debug(f"⏭ Skip empty ID (hash:{h})")
        return
    ...
    for att in attachments:
        ...
        await tg_client.send_media(file_bytes, tg_type, filename, caption=text if tg_type != "Document" else "")
        await asyncio.sleep(0.5)
    
    logger.info(f"✅ Processed: {msg_id}")

# ===================================================================
# 7. POLLING LOOP (С ИСПРАВЛЕНИЕМ ДУБЛЕЙ)
# ===================================================================
async def polling_loop():
    logger.info("🔄 Polling started...")
    
    # 🔹 Синхронизация: кешируем последние 10 сообщений при старте
    logger.info("⏳ Sync: caching last 10 messages...")
    init_msgs = await max_client.get_messages(limit=10)  # ← БЫЛО: limit=1
    for msg in init_msgs:
        _processed_hashes.add(get_msg_hash(msg))
    logger.info(f"📍 Cached hashes at startup: {len(_processed_hashes)}")
